...
import { Component, inject, signal, effect, ViewChild, ElementRef, OnInit, ChangeDetectionStrategy, OnDestroy, AfterViewInit } from '@angular/core';
...
@Component({
  ...
  <div class="flex items-center gap-4" >
    <span class="text-sm font-bold uppercase tracking-[0.25em]" > {{ langService.currentLang() === 'ar' ? 'تركيب في 3 ثواني' : 'APPLY IN 3 SECONDS' }
} </span>
...
@if (selectedBundle() && selectedBundle()!.compareAtPrice > selectedBundle()!.price) {
  <span class="text-gray-400 line-through text-sm" > {{ currencyService.formatPrice(selectedBundle()!.compareAtPrice / selectedBundle()!.quantity) }
} </span>
  ...
  @if (currentProduct.bundles && currentProduct.bundles.length > 0) {
    <div class="space-y-4 mb-10" >
      <div class="flex items-center justify-between mb-3 px-1" >
        <span class="text-xs font-black uppercase tracking-[0.2em] text-black block" > {{ langService.currentLang() === 'ar' ? 'اختاري عرضكِ المثالي' : 'SELECT YOUR BUNDLE' }
  } </span>
    < span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest" > {{ langService.currentLang() === 'ar' ? 'تخصيص الألوان متاح' : 'COLOR CUSTOMIZATION' }
} </span>
  </div>
...
@if (bundle.compareAtPrice > bundle.price) {
  <div class="text-[10px] text-gray-900 line-through font-medium mt-1 opacity-80" > {{ currencyService.selectedCurrency().symbol }
} { { bundle.compareAtPrice } } </div>
                                  }
</div>
  </div>
        ...
        <div class="flex items-center gap-2" >
          <div class="w-7 h-7 rounded border border-[#f7e8da] bg-white overflow-hidden" >
            <img[src]="giftImage()" class="w-full h-full object-cover" alt = "Free Gift" >
              ...
              < span class="text-[10px] font-bold text-gray-900" > + {{ langService.currentLang() === 'ar' ? 'هدية مجانية: مكبس رموش' : 'FREE gift: Eyelash Curler' }
    } </span>
      </div>
...
class="absolute inset-0 w-full h-full opacity-0 cursor-ew-resize z-30 m-0 p-0" >
  ...
  < !--How To Use Section-- >
    <div class="mt-8 pt-8 border-t border-gray-100 max-w-4xl mx-auto px-6" >
      <div class="text-center mb-12" >
        <h3 class="text-3xl md:text-5xl font-serif text-black mb-4" >
          {{ langService.currentLang() === 'ar' ? 'طريقة الاستخدام' : 'How To Use' }}
</h3>
  < p class="text-gray-500 uppercase tracking-widest text-[10px] font-black" >
    {{ langService.currentLang() === 'ar' ? 'بسيطة وسهلة في ثوانٍ' : 'Simple & Easy in Seconds' }}
</p>
  </div>

  < div class="relative w-full aspect-[9/16] md:aspect-video rounded-[2.5rem] overflow-hidden shadow-2xl border-4 border-white bg-gray-100" >
    <video 
                  #explanationVideo
src = "/assets/explanation%20vid.mp4"
class="absolute inset-0 w-full h-[120%] -top-[20%] object-cover object-center pointer-events-none"
autoplay
loop
muted
playsinline >
    ...
    <button (click)="openNewsletter()" class="fixed bottom-24 left-4 md:bottom-10 md:left-10 z-[1000] bg-white text-black pl-2 pr-6 py-2 rounded-full shadow-2xl flex items-center gap-4 animate-in slide-in-from-bottom duration-500 hover:scale-105 active:scale-95 transition-all border border-gray-100 group cursor-pointer hover:shadow-[0_20px_50px_rgba(0,0,0,0.1)]" >
      <div class="w-10 h-10 bg-black rounded-full flex items-center justify-center text-lg animate-pulse shadow-lg text-white" >🎁</div>
        < div class="flex flex-col text-start" >
          <span class="text-[10px] font-black uppercase tracking-widest text-black leading-tight" > {{ langService.currentLang() === 'ar' ? 'خصم ١٠٪' : '10% OFF' }
} </span>
...
export class ProductDetailComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  @ViewChild('explanationVideo') explanationVideo!: ElementRef<HTMLVideoElement>;
  ...
  constructor() {
    // Auto-play video when product loads and video element becomes available
    effect(() => {
      const p = this.product();
      if (p) {
        setTimeout(() => {
          if (this.explanationVideo?.nativeElement) {
            this.explanationVideo.nativeElement.muted = true;
            this.explanationVideo.nativeElement.play().catch(e => console.log('Autoplay prevented:', e));
          }
        }, 500);
      }
    });
  ...
  ngAfterViewInit() {
    // Initial attempt to play if already in view (fallback)
    setTimeout(() => {
      if (this.explanationVideo?.nativeElement) {
        this.explanationVideo.nativeElement.muted = true;
        this.explanationVideo.nativeElement.play().catch(() => { });
      }
    }, 1000);
  }
